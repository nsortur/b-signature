...
DsJwtAuth.prototype.checkToken = function _checkToken(bufferMin = tokenReplaceMinGet) {
    let noToken = !this.accessToken || !this._tokenExpiration
        ...
        ;
    if (this._debug) {
        if (noToken) {if (!this._debug) {return}
        console.log(this._debug_prefix + ': ' + 'checkToken: Starting up--need a token')}
        if (needToken && !noToken) {if (!this._debug) {return}
        console.log(this._debug_prefix + ': ' + 'checkToken: Replacing old token')}
        if (!needToken) {if (!this._debug) {return}
        console.log(this._debug_prefix + ': ' + 'checkToken: Using current token')}
    }
...
DsJwtAuth.prototype.getToken = async function _getToken() {
    ...
    console.log(`dsOauth: ${newServer}`)
    console.log(`clientid: ${dsConfig.dsClientId}`);
    console.log(`impersonated: ${dsConfig.impersonatedUserGuid}`);
    console.log(`rsakey: ${rsaKey}`);
    console.log(`jwtlife: ${jwtLifeSec}`);
    const results = await dsApi.requestJWTUserToken(dsConfig.dsClientId,
        dsConfig.impersonatedUserGuid, "signature", rsaKey,
        jwtLifeSec);
    console.log('reached');
 ...
 * @promise
 */
DsJwtAuth.prototype.getUserInfo = async function _getUserInfo(token){
    // Data used:
    ...
    console.log(`results: ${results.accounts}`);
    console.log(`results type ${typeof results.accounts}`);
    const accountInfo = results.accounts.find(account =>
    ...
    .catch(e => {
        ...
        if (body) {
            ...
            if (body.error && body.error === 'consent_required') {
                ...
                console.log(this._debug_prefix + ': ' + `\nAPI problem: Status code ${e.response.status}, message body:
