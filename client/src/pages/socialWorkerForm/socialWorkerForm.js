...
class SocialWorkerForm extends React.Component {
  ...
  async runSigning (event) {
    ...
    // validate email
    if (!(/^[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[A-Za-z]+$/.test(this.state.socialWorkerEmail))) { 
  ...
  render() {
    ...
    switch (this.state.step) {
      case 1:
        curForm = <SocialWorkerInfo
                handleChange = {this.handleInputChange}
                nextPage = {this.nextPage}
                values = {this.state}/>
        break;
      case 2:
        curForm = <ChildMedicalInfo
                handleChange = {this.handleInputChange}
                nextPage = {this.nextPage}
